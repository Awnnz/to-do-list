...
import { closeOverlay, closeEditOverlay } from '../pages/overlay';
...
const editForm = (section, obj, card) => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const UpdatedObjData = extractFormData(
...
function createForm(section) {
    ...
    div.addEventListener('click', function () {
      (() => {
        ...
        sidebarSections.forEach((section) => {
          if (section.classList.contains('sidebar-selected')) {
            section.classList.remove('sidebar-selected');
          }
        });
      })();
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (sectionSelected) {
      sectionSelected = document
        .querySelector('.sidebar-selected')
        .innerHTML.toLowerCase();

      UpdateTaskCountDisplay(
        sectionSelected,
        checkmarkCounterDisplay(sectionSelected),
      );
    ...
    const objData = addNewTask(
      extractFormData([title, description, due, priority]),
      section,
    );
...
function extractFormData(formData, checked) {
  ...
  formData.forEach((input) => {
    if (input) formDataObj[input.name] = input.value;
  });
...
function UpdateTaskCountDisplay(section, checkmarkAmount) {
  const countDisplay = document.querySelector(`.${section}-count`);
...
function markActiveSection(section) {
  if (section.tagName === 'SPAN') {
    section.parentElement.classList.add('selected');
...
function createCard(dataObj, section) {
  ...
  detailsButton.addEventListener('click', function () {
    this.parentElement.parentElement
      ...
      .classList.toggle('card-details-hide');
  });

  cardDiv.appendChild(cardMain);
  cardDiv.appendChild(cardDetails);

  if (cardMain.children[1]) {
    cardMain.insertBefore(detailsButton, cardMain.children[1]);
  } else cardMain.appendChild(detailsButton);
  ...
  deleteButton.addEventListener('click', function () {
    ...
    toDoListCollection.getCollection(section).forEach((element, index) => {
      if (
        this.parentElement.querySelector('.title-value').textContent
          === element.title
        && this.parentElement.querySelector('.due-value').textContent
          === element.due
      ) {
        objectIndex = index;
      }
...
const checkmarkCounterDisplay = function (section) {
  ...
  collectionArr.forEach((obj) => {
    if (obj.checked === true) {
      if (finishedTasksAmount === undefined) finishedTasksAmount = 1;
      else {
